Component({
  ...
  methods: {
	...
	// 分类
	goClassification:function(data){
		 console.log('点击的分类',data.currentTarget.dataset.index)
		 this.setData({
			 actnum:data.currentTarget.dataset.index
		 })
		 this.triggerEvent('goClassification',data.currentTarget.dataset.index)
	},
    getSystemInfo() {
      var app = getApp();
